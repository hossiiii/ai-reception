...
class LLMTestRunner:
    ...
    def _find_scenario(self, scenario_id: str) -> Optional[Dict[str, Any]]:
        """シナリオIDから該当シナリオを検索"""
        for category, scenarios in self.test_scenarios.get("test_scenarios", {}).items():
    ...
    async def run_and_report(self, scenario_ids: Optional[List[str]] = None, output_file: Optional[str] = None):
        """テスト実行とレポート生成"""
        print("🚀 LLMテストスイート開始")
        print("=" * 50)
